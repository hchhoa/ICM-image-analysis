...
if __name__ == "__main__":
    ...
    # tumor_analyser = TumorAnalyser("data/Multiplexing image_cancer-inflamation.tif")
    # tumor_analyser.load_image()
    # preprocessed_image = tumor_analyser.preprocess_image(
    #     tumor_analyser.image[:, :, 5], kernel_size=(7, 7), plot=True
    # )
    # mask = tumor_analyser.segment_image(
    #     preprocessed_image,
    #     thresh_type="global",
    #     thresh=0.06,
    #     block_size=21,
    ...
    # closed_mask = tumor_analyser.perform_closing(mask, disk_size=8, plot=True)
    # cleaned_labeled_mask = tumor_analyser.clean_label_mask(
    #     closed_mask, min_object_area=500, plot=True
    # )
    # tumor_mask = (cleaned_labeled_mask > 0) * 1
    # plot_image(tumor_mask)

    # stroma_mask = (tumor_mask == 0) * 1
    # plot_image(stroma_mask)

    # ### Question 2: inflammatory cells

    # inflammatory_cells_masks = []
    # for channel in range(5):
    #     preprocessed_image = tumor_analyser.preprocess_image(
    #         tumor_analyser.image[:, :, channel] * stroma_mask,
    #         blur=False,
    #         plot=False,
    #     )
    ...
    #     watershed_image = tumor_analyser.watershed_algo(
    #         mask, min_distance=5, min_object_area=20, plot=True
    #     )
    #     # cells_analyser.plot_boundaries(watershed_image)
    #     inflammatory_cells_masks.append(watershed_image)

    # ### Question 3: Attribute phenotype

    # # Assuming you have individual labeled masks as labeled_masks (a list of NumPy arrays)
    # composite_mask = tumor_analyser.combine_masks(inflammatory_cells_masks)

    # preprocessed_image = tumor_analyser.preprocess_image(
    #     tumor_analyser.image, plot=False
    # )

    # cells_expression = tumor_analyser.analyze_marker_expression(
    #     composite_mask, preprocessed_image
    # )

    # attributed_phenotypes = tumor_analyser.attribute_phenotypes(cells_expression)

    # ### Question 4:
    # # Count the occurrences of each phenotype
    # proportions = tumor_analyser.count_phenotypes(attributed_phenotypes)

    # # Print the proportions
    # for phenotype, proportion in proportions.items():
    #     print(f"{phenotype}: {proportion}")

    # ## Question 5:
    # mean_minimal_distances = tumor_analyser.compute_min_distance(
    #     stroma_mask, composite_mask, attributed_phenotypes
    # )
    # mean_minimal_distances
    # # [45.254915583840365, 30.723161179970745, 49.927262455018855, 38.195564676444, 32.9678816283516]
